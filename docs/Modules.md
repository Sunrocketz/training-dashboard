...
| Overview tab | Баннер периода, KPI+MoM с пояснениями, воронка, отсев, тренд, топы | Ломать взвешенную/среднюю формулы без ApiContract |
...
- `renderAll` / `renderOverview` / `renderTrainersTab` / `renderMonthsTab`
- `openTrainerDetail` / `openMonthDetail` — сводки из `groups` + отсев
...
|--------|----------|-------------|
| Google Sheet | Операционная команда | Имена листов, колонки A–R |
