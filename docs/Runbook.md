...
5. `unmatched` большой — имена в журнале не клеятся с «Контроль штата» (фамилия + имя; `ё` уже равен `е`).
...
- [ ] UI: рейтинг TQI v3, колонка «Оценка кандидатов» (скрипт / возражения / CRM); в JSON есть `scoreParts.skills`
- [ ] Карточка тренера: блок «После линии» (средняя 1–5, скрипт/возражения/CRM)
- [ ] «Дегтярёва Надежда» имеет `lineReview` (склейка ё/е)
- [ ] Селиванова и Вовчак нет в `byTrainer` / рейтинге / таблице линии; `metrics.excludedTrainers` есть
